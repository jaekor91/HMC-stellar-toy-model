...
for mag_model in mag_arr:
    ...
    alpha = np.array([1e-3, 1e-2, 1e-2])

    #---- *Time* evolution where step sizes are adjusted by flux.
    # Compute intial time steps
    dt_f =  1e-2 * phi_t[0, 0] # Fraction of previous flux
    dt_xy = 1./phi_t[0, 0] # Inverse of previous flux
    ...
    grad = dVdq(phi_t[i, :])
    grads_t[i]= grad
    p_t[i, :] = - dt * grad / 2. # We assume unit covariance momentum matrix.
    ...
    for i in range(1, Nsample):
        ...
        dt_f = 1e-2 * phi_t[i, 0] # Fraction of previous flux
        dt_xy = 1./phi_t[i, 0] # Inverse of previous flux
        dt = np.array([dt_f, dt_xy, dt_xy])
        dt_t[i] = dt
        ...
        grad = dVdq(phi_t[i])
        grads_t[i]= grad    
        p_t[i] = (1.-alpha) * p_t[i-1] - dt * grad

    plt.close()
    fig, ax_list = plt.subplots(6, 2, figsize=(25, 20))
    #---- xy
    # xy
    ax_list[0, 0].plot(range(Nsample), phi_t[:, 1], label="x", c="black")
    ax_list[0, 0].plot(range(Nsample), phi_t[:, 2], label="y", c="red")
    ax_list[0, 0].set_ylabel("xy", fontsize=20)
    # Time step xy
    ax_list[1, 0].plot(range(Nsample), dt_t[:, 1], label="x", c="black")
    ax_list[1, 0].set_ylabel("dt_xy", fontsize=20)
    # Gradient xy
    ax_list[2, 0].plot(range(Nsample), grads_t[:, 1], label="x", c="black")
    ax_list[2, 0].plot(range(Nsample), grads_t[:, 2], label="y", c="red")
    ax_list[2, 0].set_ylabel("dV/dxy", fontsize=20)
    # Gradient * time step
    ax_list[3, 0].plot(range(Nsample), grads_t[:, 1] * dt_t[:, 1], label="x", c="black")
    ax_list[3, 0].plot(range(Nsample), grads_t[:, 2] * dt_t[:, 2], label="y", c="red")
    ax_list[3, 0].set_ylabel("dt_xy * dV/dxy", fontsize=20)
    # Momentum xy
    ax_list[4, 0].plot(range(Nsample), p_t[:, 1], label="x", c="black")
    ax_list[4, 0].plot(range(Nsample), p_t[:, 2], label="y", c="red")
    ax_list[4, 0].set_ylabel("p_xy", fontsize=20)
    # Momentum xy * time_step
    ax_list[5, 0].plot(range(Nsample), p_t[:, 1]* dt_t[:, 1], label="x", c="black")
    ax_list[5, 0].plot(range(Nsample), p_t[:, 2]* dt_t[:, 2], label="y", c="red")
...
#     plt.show()
    plt.savefig(dir_figures+"one-star-in-background-mag%d-single-history.png" % mag_model, dpi=200, bbox_inches="tight")
    plt.close()        
print "Completed"