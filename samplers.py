...
class RHMC_GMM(object):
    ...
    def V_and_derivatives_stable(self, q):
        ...
        V = -gamma_max - np.log(np.sum(self.A_ls * gamma_diff_exp))

        # Compute the ratio pi_l / pi
        r = gamma_diff_exp / np.sum(self.A_ls * gamma_diff_exp)

        # print "gamma", gamma
        # print "gamma_max", gamma_max
        # print "gamma_diff", gamma_diff
        # print "A_ls * gamma_diff", self.A_ls * gamma_diff
        # print "r", r
    ...
    def RHMC_single_simulator(self, q0, Nsteps = 100, eps=0.5, alpha=1e6, delta = 1, p=None, debug=False): 
        ...
        for i in xrange(1, Nsteps+1):
            ...
            # First momentum step
            V, dVdq, H_ii, dH_iidq = self.V_and_derivatives_stable(q)
            grad = self.dphi_dq(H_ii, dH_iidq, alpha, dVdq)
            p -= (eps/2.) * grad 
            ...
            V, dVdq, H_ii, dH_iidq = self.V_and_derivatives_stable(q)
            while Dp > delta:
                grad = self.dtau_dq(p, H_ii, dH_iidq, alpha)
            ...
            V, dVdq, H_ii, dH_iidq = self.V_and_derivatives_stable(q)
            p -= (eps/2.) * self.dtau_dq(p, H_ii, dH_iidq, alpha)
            if np.any(np.isnan(p)) or np.any(np.isnan(q)):
                print "/--4"
                print i
                print "q", q, Dq
            ...
            p -= (eps/2.) * self.dphi_dq(H_ii, dH_iidq, alpha, dVdq)
            if np.any(np.isnan(p)) or np.any(np.isnan(q)):
                print "/--5"
                print i
                print "q", q, Dq
